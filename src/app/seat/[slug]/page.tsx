...
export default function SeatFinderPage() {
    ...
    return (
        <div
            ...
            style={{ background: `linear-gradient(180deg, #ffffff 0%, ${bg} 100%)` }}
        >
            <div className="w-full max-w-md">
                ...
                <header className="text-center mb-8">
                    ...
                    {info ? (
                        <h1 style={{ fontFamily: SCRIPT, color: accent }} className="text-5xl sm:text-6xl leading-none mb-2">
                            {info.bride} &amp; {info.groom}
                        </h1>
                    ) : (
                        <div className="h-14 w-3/4 mx-auto rounded-lg animate-pulse bg-stone-200 mb-2" />
                    )}
                    ...
                    <div className="flex items-center justify-center gap-3">
                        <div className="h-px w-10 opacity-20" style={{ backgroundColor: accent }} />
                        <span className="text-sm opacity-40" style={{ color: accent }} aria-hidden="true">✦</span>
                        <div className="h-px w-10 opacity-20" style={{ backgroundColor: accent }} />
                    </div>
                </header>
                ...
                {/* ── Search card ── */}
                <div
                    className="bg-white rounded-2xl p-5 shadow-[0_8px_32px_rgba(0,0,0,0.08)] border mb-5"
                    style={{ borderColor: `${accent}20` }}
                >
                    <p className="text-[10px] tracking-[0.2em] uppercase font-medium mb-3" style={{ color: accent }}>
                    ...
                    <form onSubmit={handleSearch} className="space-y-3">
                        <div className="relative">
                            ...
                            <input
                                ...
                                autoFocus
                                className="w-full pl-10 pr-4 py-3.5 rounded-xl border text-stone-800 text-[15px] outline-none transition-shadow min-h-[44px]"
                ...
                {searched && !loading && results.length === 0 && !error && (
                    <div
                        className="bg-white rounded-2xl p-8 text-center border shadow-sm"
                        style={{ borderColor: `${accent}15` }}
                    >
                        <p style={{ fontFamily: SERIF, color: accent }} className="text-xl italic font-light mb-2">
                            No match found
                        </p>
                ...
                <div className="space-y-4">
                    {results.map((r, i) => (
                        <div
                            key={`${r.guestName}-${i}`}
                            className="rounded-2xl bg-white overflow-hidden border shadow-[0_8px_32px_rgba(0,0,0,0.07)] animate-in fade-in slide-in-from-bottom-2 duration-500"
                            style={{ borderColor: `${accent}20` }}
                        >
                            {/* Gradient header */}
                            <div
                            ...
                            <div className="px-6 py-5">
                                {r.table ? (
                                    <>
                                        <p style={{ fontFamily: SCRIPT, color: accent }} className="text-5xl leading-none mb-1">
                                            {r.table.name}
                                        </p>
                                        <p className="text-[10px] tracking-[0.15em] uppercase font-medium opacity-50 mb-4" style={{ color: accent }}>
                                ...
                                {r.tablemates.length > 0 && (
                                    <div className="pt-4 border-t" style={{ borderColor: `${accent}15` }}>
                                        <p className="text-[9px] tracking-[0.2em] uppercase font-medium mb-3 opacity-50" style={{ color: accent }}>
                                            You&apos;ll be seated with
                                        </p>
                                        <div className="space-y-1.5">
                                            {r.tablemates.map((name, ni) => (
                                                <p key={ni} style={{ fontFamily: SERIF, color: accent }} className="text-[15px] font-light italic">
                                                    &mdash; {name}
                                                </p>
                                            ))}
                                        </div>
                ...
                <div className="mt-16 text-center">
                    <p className="text-[9px] tracking-[0.3em] uppercase font-light opacity-25" style={{ color: accent }}>
